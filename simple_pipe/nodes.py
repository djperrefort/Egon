...
class NodeBase:
    ...
    def input_connections(self) -> List[connectors.Input]:
        """Returns a list of input connections assigned to the current node
    ...
    def output_connections(self) -> List[connectors.Output]:
        """Returns a list of output connections assigned to the current node
    ...
    def input_nodes(self) -> Union[Source, Inline]:
        """Returns a list of upstream pipeline nodes connected to the current node"""

        nodes = [c.node for c in self.input_connections()]
        return cast(Union[Source, Inline], nodes)

    def output_nodes(self) -> Union[Inline, Target]:
        """Returns a list of downstream pipeline nodes connected to the current node"""

        nodes = [c.node for c in self.output_connections()]
...
class Source(NodeBase, metaclass=abc.ABCMeta):
...
class Target(NodeBase, metaclass=abc.ABCMeta):
